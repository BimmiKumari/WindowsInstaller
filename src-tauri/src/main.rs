...
/// Constructs the download URL dynamically based on the tool name, version, architecture, and OS.
...
/// Downloads and installs the tool with progress reporting
fn download_and_install_with_progress(
    ...
    loop {
        ...
        // Update progress and speed calculations every 500ms
        let now = Instant::now();
    ...
    // Execute installer if it's an executable
...
fn check_java_version() -> Result<String, String> {
    ...
    // Extract version from output (e.g., "java version "11.0.12"")
...
fn check_kubernetes_version() -> Result<String, String> {
    ...
    // Extract version from output (e.g., "Client Version: v1.21.0")
    let version = version_output
        .lines()
...
fn check_docker_version() -> Result<String, String> {
    ...
    // Extract version from output (e.g., "Docker version 20.10.12, build...")
    let version = version_output
        .split(',')
        .next()
        .and_then(|v| {
...
fn check_git_version() -> Result<String, String> {
    ...
    // Extract version from output (e.g., "git version 2.30.2")
    let version = version_output
        .split_whitespace()
        .nth(2)
        .map(|v| v.to_string())
...
fn check_openapi_generator_version() -> Result<String, String> {
    let output = Command::new("openapi-generator")
        .arg("version")
        .output()
        .map_err(|e| format!("Error running openapi-generator version: {}", e))?;
    
    let version_output = String::from_utf8_lossy(&output.stdout).to_string();
    
    // Extract version from output (e.g., "5.4.0")
    let version = version_output
        .lines()
        .next()
        .map(|v| v.trim().to_string())
        .ok_or_else(|| "Could not parse OpenAPI Generator version".to_string())?;
